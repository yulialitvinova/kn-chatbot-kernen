...
# from llama_index import VectorStoreIndex, KeywordTableIndex #SimpleDirectoryReader, BasePromptTemplate, 
...
##### share my app: https://docs.streamlit.io/deploy/streamlit-community-cloud/share-your-app
...
vectorstore = Chroma(
    ...
    persist_directory="./99_chroma_db_kernen"
...
QA_CHAIN_PROMPT_spec_search = PromptTemplate(
    template=template_query_spec_search, input_variables=["summaries", "question"] # , "link"]
    )


text_splitter_urls_sugg = RecursiveCharacterTextSplitter(chunk_size=200, chunk_overlap=50)
vectorstore_urls_sugg = Chroma(
    embedding_function=embeddings_model,
    persist_directory="./99_chroma_db_kernen_urls"
    )
...
template_query_urls_sugg="""<<SYS>> \n You are an assistant to citizens in difficult situations. \
        You need to find the relevant url (i.e., Document(metadata=source)) in DOCUMENTS, based on the Document(page_content) extracted above.\
        If you do not know the answer reply with 'Es tut mir Leid, ich habe nicht genügend Informationen'.\
...
agent.agent.llm_chain.prompt.template = """
    ...
    Action Input: the input to the action, input to the tool: {input}
